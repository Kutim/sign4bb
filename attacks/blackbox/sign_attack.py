...
class SignAttack(BlackBoxAttack):
    ...
    def _suggest(self, xs_t, loss_fct, metric_fct):
        ...
        if self.is_new_batch:
            ...
            self.sgn_t = sign(ch.randn(_shape[0], dim))
            self.h = 0
            self.i = 0
            add_queries = 1
        ...
        fxs_t = self.xo_t + self.fd_eta * self.sgn_t.view(_shape)
        ...
        #print(cos_sims, ham_sims)
        # perform the step
        new_xs = linf_step(xs_t, self.sgn_t.view(_shape), self.epsilon)
        # update i and h for next iteration
        self.i += 1
        if self.i == 2 ** self.h or istart >= dim:
            self.h += 1
            self.i = 0
            ...
            if self.h == np.ceil(np.log2(dim)).astype(int) + 1:
                self.xo_t = xs_t
                self.h = 0
                add_queries = 1 # because it will be queried next round at bxs_t
                print("new change")
